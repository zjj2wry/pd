...
func (s *testVersionSuite) TestGetVersion(c *C) {
	...
	go func(cfg *config.Config) {
		...
		c.Assert(err, IsNil)
		c.Assert(failpoint.Enable("github.com/pingcap/pd/v3/server/memberNil", `return(true)`), IsNil)
		reqCh <- struct{}{}
		err = s.Run()
		c.Assert(err, IsNil)
		ch <- s
	...
	c.Assert(failpoint.Disable("github.com/pingcap/pd/v3/server/memberNil"), IsNil)
}
